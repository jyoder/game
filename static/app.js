...
Read more about this issue at https://excaliburjs.com/docs/performance`);
  ...
  function run() {
    ...
    const ball = new __webpack_exports__Actor({
      x: 100,
      y: 300,
      radius: 10,
      color: __webpack_exports__Color.Red
    });
    const ballSpeed = __webpack_exports__vec(300, 300);
    setTimeout(() => {
      ball.vel = ballSpeed;
    }, 1e3);
    ball.body.collisionType = __webpack_exports__CollisionType.Passive;
    game.add(ball);
    ball.on("postupdate", () => {
    ...
    const columns = 5;
    const rows = 3;
    const brickColor = [__webpack_exports__Color.Violet, __webpack_exports__Color.Orange, __webpack_exports__Color.Yellow];
    const brickWidth = game.drawWidth / columns - padding - padding / columns;
    const brickHeight = 30;
    const bricks = [];
    for (let j = 0; j < rows; j++) {
      for (let i = 0; i < columns; i++) {
        bricks.push(new __webpack_exports__Actor({
    ...
    bricks.forEach(function(brick) {
      brick.body.collisionType = __webpack_exports__CollisionType.Active;
      game.add(brick);
    });
    let colliding = false;
